...
namespace PDFSender {
  ...
  export function createAndSendPDFsForZoneOrDistrict(
    ...
    try {
      ...
      Logger.log(`ERROR - error occurred while creating and sending pdfs for the ${name} ${
        isZone ? "zone" : "district"
      }. skipping...
  ...
  function getEmailsInZoneOrDistrict(name: string, isZone: boolean): string[] {
    try {
      ...
      const filtered: string[] = dataRangeValues
        .filter(
          (row) =>
            row[zoneOrDistrictCol].toString().toLowerCase() ==
            name.toLowerCase()
        )
      ...
      Logger.log(`ERROR - error occurred while getting emails for the ${name} ${
        isZone ? "zone" : "district"
      }. skipping...
            Error Message: ${e.toString()}`);
      return [];
  ...
  function getLeaderEmailsForZoneOrDistrict(
    ...
    try {
      ...
      Logger.log(`ERROR - error occurred while getting leader emails for the ${name} ${
        isZone ? "zone" : "district"
      }. skipping...
          Error Message: ${e.toString()}`);
      return [];
  ...
  function copyFilteredDataToSheet(
    ...
    try {
      ...
      Logger.log(`ERROR - error occurred while copying data into the filtered sheet. skipping...
        Error Message: ${e.toString()}`);
    }
  }
  ...
  function getSpreadsheetAsPDF(
    ...
    try {
      ...
      Logger.log(`ERROR - error occurred while exporting pdf ${pdfName}. skipping...
        Error Message: ${e.toString()}`);
      return null;
    }
  }
  ...
  function emailPDFs(
    ...
    try {
      ...
      Logger.log(`ERROR - error occurred while sending pdfs to ${email}. skipping...
        Error Message: ${e.toString()}`);
    }
  }
}
