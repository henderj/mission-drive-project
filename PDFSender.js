function createAndSendPDFs() {
  const zones = getZoneRange().getValues().map(row => row[0].toString()).filter(zone => zone != '');
  zones.forEach(zone => {
    createAndSendPDFsForZoneOrDistrict(zone, true)

    const districts = getDistrictRange(zone).getValues()[0];
    districts = districts.map(col => col.toString()).filter(district => district != '');
    districts.forEach(district => createAndSendPDFsForZoneOrDistrict(district, false));
  });
}
...
function createAndSendPDFsForZoneOrDistrict(name, isZone) {
  ...
  copyFilteredDataToSheet(getEmailsInZoneOrDistrict(name, isZone), getTalentFilteredDataSheet());
  ...
  const fullInfoPDFName = `${name} ${isZone ? 'Zone' : 'District'} Missionary Talents`;
  ...
  var body = `Here are two autogenerated pdf files that contain the talents of the missionaries in your ${isZone ? 'zone' : 'district'}! :)`;
  ...
  const fullInfoPDF = getSpreadsheetAsPDF(getTalentSpreadsheetID(), fullInfoSheetID, fullInfoPDFName, fullInfoRange);
  fullInfoSheet.hideSheet();

  quickInfoSheet.showSheet();
  const quickInfoPDF = getSpreadsheetAsPDF(getTalentSpreadsheetID(), quickInfoSheetID, quickInfoPDFName, null, "A4");
  ...
  leaderEmails.forEach(email => {
    ...
    Logger.log("***DEBUGGING NOTICE*** Would be sending email to %s. Rerouting to %s for testing reasons...", email, reroute);
...
function getEmailsInZoneOrDistrict(name, isZone) {
  ...
  const zoneOrDistrictCol = (isZone ? getZoneColNum() : getDistrictColNum()) - 1;
  const emailCol = getEmailAddressColNum() - 1;
  const filtered = dataRange.filter(row => row[zoneOrDistrictCol].toString().toLowerCase() == name.toLowerCase())
    .map(row => row[emailCol].toString().toLowerCase().trim());
  Logger.log(`emails in ${name} ${isZone ? 'zone' : 'district'}:\n${filtered.join('\n')}`);
  return filtered;
}
...
function getLeaderEmailsForZoneOrDistrict(name, isZone) {
  ...
  const filtered = dataRange.filter(row => row[zoneOrDistrictCol].toString().toLowerCase() == name.toLowerCase())
    .filter(row => {
      ...
      return isZone ? (access == "ZL" || access == "STL" || access == "SMS") : (access == "DL");
    })
    .map(row => row[emailCol].toString().toLowerCase().trim());
  Logger.log(`leader emails in ${name} ${isZone ? 'zone' : 'district'}:\n${filtered.join('\n')}`);
  return filtered;
}
...
function copyFilteredDataToSheet(emails, sheetToPasteIn) {
  ...
  sheetToPasteIn.getRange(2, 1, sheetToPasteIn.getMaxRows(), sheetToPasteIn.getMaxColumns()).clear();
...
function getSpreadsheetAsPDF(spreadsheetID, sheetID, pdfName, range = null, size = "A3", portrait = true) {
  const ss = SpreadsheetApp.openById(spreadsheetID);
  const url = 'https://docs.google.com/spreadsheets/d/SS_ID/export?'.replace('SS_ID', ss.getId());
  let exportOptions =
    'exportFormat=pdf&format=pdf' + // export as pdf / csv / xls / xlsx
    '&size=' + size + // paper size legal / letter / A4
  ...
  if (range != null) {
    exportOptions +=
      ...
      '&r2=' + range.getLastRow() + // End Row number
      '&c2=' + range.getLastColumn();  // End Column number
  }
