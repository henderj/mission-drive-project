...
function archiveContentFoldersIfAreaFolder(folder) {
  ...
  if (name.toLowerCase().includes(getAreaFolderSuffix().toLowerCase())) {
    Logger.log("Found Area Folder! (%s) Archiving and/or creating content folders...", name);
...
function archiveFolder(folderToArchive, areaFolder) {
  ...
  const archiveFolder = getFolder(areaFolder, getArchiveFolderName(areaFolder), true);

  const currentDateFolder = getFolder(archiveFolder, getTodayDateFormatted(), true);
