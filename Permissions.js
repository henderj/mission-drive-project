function updatePermissions() {
  ...
  const emails = range.filter(row => isMissionaryEmail(row[emailAddressColNum]))
...
function updatePermissionsToMissionDatabase(emails) {
  ...
  const currentEmails = missionDrive.getViewers().map(user => user.getEmail().toLowerCase());

  for (let i = 0; i < currentEmails.length; i++) {
    if (emails.includes(currentEmails[i])) {
      const index = emails.indexOf(currentEmails[i]);
      emails.splice(index, 1);
      Logger.log("%s is already an viewer for the %s folder. Continuing to next email...", currentEmails[i], missionDrive.getName());
      continue;
    }
    Logger.log("%s is no longer supposed to be an viewer for the %s folder. Revoking access...", currentEmails[i], missionDrive.getName());
    if (currentEmails[i] == effectiveUserEmail) {
      Logger.log("PSYCH!! %s is the email running this script. It would be very bad if I removed access from myself... Continuing to next email :)", currentEmails[i]);
      continue;
    }
  ...
  emails = emails.filter(e => missionDrive.getAccess(e) == DriveApp.Permission.NONE);

  if (emails.length <= 0) {
    Logger.log("No emails to give viewer access to. Continuing to next folder.");
    return;
  }

  Logger.log("Giving the following emails access to the %s folder:\n%s", missionDrive.getName(), emails.join('\n'));
...
function setAccessToFolder(folder, map) {
  ...
  if (!map.has(folderName)) {
    Logger.log("Access map does not have a value for folder named %s. Continuing...", folderName);
    return;
  }
  ...
  const currentEmails = folder.getEditors().map(user => user.getEmail().toLowerCase());

  for (let i = 0; i < currentEmails.length; i++) {
    if (emails.includes(currentEmails[i])) {
      const index = emails.indexOf(currentEmails[i]);
      emails.splice(index, 1);
      Logger.log("%s is already an editor for the %s folder. Continuing to next email...", currentEmails[i], folder.getName());
      continue;
    }
    Logger.log("%s is no longer supposed to be an editor for the %s folder. Revoking access...", currentEmails[i], folder.getName());
    if (currentEmails[i] == effectiveUserEmail) {
      Logger.log("PSYCH!! %s is the email running this script. It would be very bad if I removed access from myself... Continuing to next email :)", currentEmails[i]);
      continue;
    }
  ...
  emails = emails.filter(e => folder.getAccess(e) == DriveApp.Permission.NONE || folder.getAccess(e) == DriveApp.Permission.VIEW);

  if (emails.length <= 0) {
    Logger.log("No emails to give editor access to. Continuing to next folder.");
    return;
  }

  Logger.log("Giving the following emails access to the %s folder:\n%s", folder.getName(), emails.join('\n'));
...
function getAccessMap(range) {
  ...
  for (let i = 0; i < range.length; i++) {
    ...
    const district = range[i][getDistrictColNum() - 1] + getDistrictFolderSuffix();
    ...
    if (!isMissionaryEmail(email)) {
      Logger.log("%s is not a missionary email. Continuing to next row...", email);
      continue;
    }
